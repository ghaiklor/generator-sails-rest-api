...
import negotiate from '../../../api/responses/negotiate';
...
describe('responses:negotiate', () => {
  it('Should handle empty error object', () => {
    ...
    assert.ok(context.serverError.calledWith({}, {code: undefined, message: undefined, root: undefined}));
  });
  ...
  it('Should handle complex error object', () => {
    ...
    assert.ok(context.serverError.calledWith({error: 'MY_ERROR'}, {
      code: 'MY_CODE',
      message: 'MY_MESSAGE',
      root: {root: 'MY_ROOT'}
    }));
  ...
  it('Should handle error from Waterline', () => {
    ...
    assert.ok(context.badRequest.calledWith([], {code: 'E_VALIDATION', message: 'ANY_REASON', root: undefined}));
  });

  it('Should handle error from Passport', () => {
    negotiate.call(context, {name: 'InternalOAuthError', oauthError: 401, message: 'PASSPORT_ERROR'});
    assert.ok(context.unauthorized.calledWith({}, {code: undefined, message: 'PASSPORT_ERROR', root: undefined}));
  });
  ...
  it('Should handle error with unauthorized status', () => {
    ...
    assert.ok(context.unauthorized.calledWith([], {code: 'E_UNAUTHORIZED', message: 'ANY_REASON', root: undefined}));
  });
  ...
  it('Should handle error with forbidden status', () => {
    ...
    assert.ok(context.forbidden.calledWith([], {code: 'E_FORBIDDEN', message: 'ANY_REASON', root: undefined}));
  });
  ...
  it('Should handle error with notFound status', () => {
    ...
    assert.ok(context.notFound.calledWith([], {code: 'E_NOT_FOUND', message: 'ANY_REASON', root: undefined}));
  });
  ...
  it('Should handle error with badRequest status', () => {
    ...
    assert.ok(context.badRequest.calledWith([], {code: 'E_BAD_REQUEST', message: 'ANY_REASON', root: undefined}));
  });
  ...
  it('Should handle error with server error status', () => {
    ...
    assert.ok(context.serverError.calledWith([], {
      code: 'E_INTERNAL_SERVER_ERROR',
      message: 'ANY_REASON',
      root: undefined
    }));
